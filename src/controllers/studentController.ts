...
class StudentController {
    createStudent = async (
        ...
        try {
            ...
            const student = await studentsServices.findStudentByEmail(
                studentData.email
            );
    ...
    showStudents = async (
        ...
        try {
            const { page, limit, search, filter, startDate, endDate, sort } =
                req.query;
            ...
            res
                .status(200)
                .json({ message: `List of students ! ${student.length}`, student });
        } catch (error) {
            next(error);
        }
    };

    updatedStudent = async (
        req: Request,
        res: Response,
        next: NextFunction
    ): Promise<void> => {
        try {
            ...
            const updatedStudent = await studentsServices.updatedStudent(
                studentId,
                studentData,
    ...
    deleteStudent = async (
        ...
        try {
            ...
            res
                .status(200)
                .json({ message: `Student Deleted by instuctor ${instructorId}` });
        } catch (error) {
            next(error);
        }
    };

    showStudentById = async (
        req: Request,
        res: Response,
        next: NextFunction
    ): Promise<void> => {
        try {
            ...
            res
                .status(200)
                .json({ message: `Student with Id ${studentId}`, student });
        } catch (error) {
            next(error);
        }
    };
